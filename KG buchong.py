...
def mine_paths_between_nodes(Graph, user_node, movie_node, maxLen,fw_file):
    ...
    if UIUI_size > path_size:
        ...
        UIUI = UIUI[:path_size]
    if UIUI_size == 0:
        UIUI.append([user_node,'ipadding','upadding',movie_node])
    ...
    if UIAI_size == 0:
        UIAI.append([user_node,'ipadding','apadding',movie_node])
    ...
    if UIDI_size == 0:
        UIDI.append([user_node,'ipadding','dpadding',movie_node])
    ...
    if UIGI_size == 0:
        UIGI.append([user_node,'ipadding','gpadding',movie_node])
...
def dump_paths(Graph, candidate, maxLen, fw_file):
    '''
    ...
    for lines in candidate:
        turples = lines.split(',')
        user_node = turples[0]
        item_node = turples[1].replace('\n', '')

        if Graph.has_node(user_node) and Graph.has_node(item_node):
            mine_paths_between_nodes(Graph, user_node, item_node, maxLen, fw_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--negetive_paths', type=str, dest='negative_paths', default='data/negative_paths_buchong.txt')
    parser.add_argument('--positive_paths', type=str, dest='positive_paths', default='data/positive_paths_buchong.txt')
    parser.add_argument('--all_nodes', type=str, dest='all_nodes', default='data/candidate_user_items.txt')
    ...
    dump_paths(Graph, fw_all_nodes, path_length,fw_file1)
